"""
Ablation 3 전용 inference: (sketch + matte) → raw hair patch 저장.

infer.py와 달리:
  - 전체 test split 순회 (num_samples 제한 없음)
  - data["filename"] stem 기반으로 {output_dir}/{stem}.png 저장
  - raw generated patch만 저장 (composite/panel 불필요)

Usage:
  python scripts/infer_ablation_cl.py \
    --config    configs/phase2_braid.yaml \
    --checkpoint checkpoints/phase2_braid/final.pth \
    --split     braid_test \
    --output_dir custom_results/ablation_cl/c5_ours/braid
"""
...
def main():
    ...
    parser.add_argument("--split",       required=True, help="unbraid_test | braid_test")
    ...
    for idx in tqdm(range(len(dataset)), desc="Generating"):
        ...
        Image.fromarray(to_uint8(gen.cpu())).save(output_dir / f"{stem}.png")

    print(f"\n완료: {output_dir} ({len(dataset)}개)")
