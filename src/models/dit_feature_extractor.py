...
Architecture:
  hair_latent (B, 16, 64, 64)
    → SD3.5 DiT (sigma=0, null conditioning)
        blocks 0-11:  frozen base + inverse LoRA (to_q/to_k/to_v, rank-8)
        blocks 12-23: fully unfrozen (shared backbone)
...
Partial unfreeze rationale:
  Front 12 (lower-level texture): inverse LoRA-only — preserve general features.
  Back 12  (higher-level semantic): shared backbone — updated jointly by forward FM
    and inverse supervised steps in Stage 3 (bidirectional partial joint training).
...
class LoRALinear(nn.Module):
    """Inverse-only LoRA adapter on a frozen base Linear."""
...
class DiTFeatureExtractor(nn.Module):
    ...
    def __init__(
        ...
        # Inverse LoRA on front 12 blocks (0-11); back 12 blocks are fully unfrozen below
        ...
        # model.parameters() and receive gradients from both supervised and FM paths.
    ...
    def _inject_lora(self, rank: int, alpha: float) -> None:
        """Replace to_q/to_k/to_v in front NUM_FRONT_LORA blocks with LoRALinear."""
    ...
    def freeze_lora(self) -> None:
        """Freeze inverse LoRA during forward FM steps (back_blocks only update)."""
